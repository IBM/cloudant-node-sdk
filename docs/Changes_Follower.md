...
[POST `_changes` API](https://github.com/IBM/cloudant-node-sdk/tree/v0.12.25/examples#postchanges) instead of the follower.
...
In these use-cases use the SDK's [POST `_changes` API](https://github.com/IBM/cloudant-node-sdk/tree/v0.12.25/examples#postchanges)
...
```ts
const latestSeq = changesFollower.latestSequenceFrom(checkpointSequenceId);
```
