...
// Setup params
const paginationParams: CloudantV1.PostPartitionSearchParams = {
...
// Option: stream rows
const rowStream: Stream<CloudantV1.SearchResultRow> = pagination.rowStream(); // a new stream of the rows
...
const pager: Pager<CloudantV1.SearchResultRow> = pagination.pager();
(async () => {
  if (pager.hasNext()) {
    const page: Array<CloudantV1.SearchResultRow> = await pager.getNext();
    // Do something with page
  }
})();
