...
set -ev
file="./scripts/build.json"
printf "Current values set:\n Module ID: $MODULE_ID\nBuild name: $BUILD_NAME\nBuild URL: $BUILD_URL\nBuild timestamp: $BUILD_TIMESTAMP\nBuild number: $BUILD_NUMBER\n"
printf "Artifactory URL: $ARTIFACT_URL\n"
ARTIFACTS=$(curl -H "X-JFrog-Art-Api:$ARTIFACTORY_APIKEY" "$ARTIFACT_URL")
...
if [[ "$HAS_CHILDREN_RESULT" == "false" ]]; then
  ARTIFACTS_URI=$(echo $ARTIFACTS | jq -r '.uri')
else 
...
CURRENT_BUILD=$(curl -H "X-JFrog-Art-Api:$ARTIFACTORY_APIKEY" "$STAGE_ROOT"build/"$BUILD_NAME"/"$BUILD_NUMBER")
echo $CURRENT_BUILD
...
curl -i -X PUT -H "X-JFrog-Art-Api:$ARTIFACTORY_APIKEY" -H "Content-Type: application/json"  "$STAGE_ROOT"build  --upload-file $file
